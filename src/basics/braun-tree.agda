...
bt-insert a (bt-node{n}{m} a' l r _) | inj₁ p | (a1 , a2) rewrite p
  = bt-node a1 (bt-insert a2 r) l (inj₂ refl)
...
bt-delete-min (bt-node a (bt-node{n}{m} x l1 r1 u1) (bt-node{n'}{m'} x' l2 r2 u2) u)
  ...
  = if (x <A x')
     then (bt-node x (bt-node x' l2 r2 u2)
                     (bt-delete-min (bt-node x l1 r1 u1))
                     ((lem{n}{m}{n'}{m'} u)))
     else (bt-node x' (bt-replace-min x (bt-node x' l2 r2 u2))
                     (bt-delete-min (bt-node x l1 r1 u1)) (lem{n}{m}{n'}{m'} u)) 
    where
      lem : {n m n' m' : ℕ} → suc (n + m) ≡ suc (n' + m')
                              ∨ suc (n + m) ≡ suc (suc (n' + m'))
                              → suc (n' + m') ≡ n + m
                              ∨ suc (n' + m') ≡ suc (n + m)
